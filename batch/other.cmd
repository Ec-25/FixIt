...
if "%tool%" == "1" (
    ...
    @REM without policy restrictions creates a system restore point, so that if something goes wrong the computer is not affected
    ...
    echo.
    @REM windows services initially considered unnecessary for average users
    ...
    sc stop PcaSvc& sc config PcaSvc start= demand
...
if "%tool%" == "3" (
    @REM Uninstall the automatically indicated system-owned apps with all necessary permissions.
    echo.
    ...
    goto tl4
    :4op4.next
    @REM set the execution policies to unrestricted to be able to manipulate system applications without inconvenience
    PowerShell Set-ExecutionPolicy Unrestricted
    echo.
    set /p appak=APPID=
    @REM Finally, the execution restriction is placed again since otherwise it would be dangerous to leave it unlimited since anything could be installed on the computer without you knowing it
...
if "%tool%" == "4" (
    @REM download and run the office uninstall tool, since many times its internal uninstaller leaves residual files and keys that prevent the installation of a new instance.
...
if "%tool%" == "5" (
    echo.
    PowerShell Set-ExecutionPolicy Unrestricted
    @REM register a new application installation that has windows installer
...
if "%tool%" == "6" (
    ...
    set /p appname=APPID=
    @REM and install the ones the user chooses
...
if "%tool%" == "7" (
    @REM download and install the most recent video drivers with a web request, this is because many people use formats that are not natively recognized by the system, to facilitate their installation
...
if "%tool%" == "8" (
    REM From the Microsoft servers download an office iso with a trial version, and if the user has an active license all the features
...
if "%tool%" == "10" (
    ...
    @REM set the execution privacy registry setting to enabled
...
if "%tool%" == "11" (
    ...
    @REM set the registry configuration of the execution privacy section to disabled
...
if "%tool%" == "15" (
    ...
    @REM scans the disk for deleted files where the user indicates in filter
